...
class FinesListCreate(APIView):
    """
    List all user profiles or create a new user profile.
    """
    def get(self, request, format=None):
        user_profiles = Fines.objects.all()
        serializer = FinesSerializer(user_profiles, many=True)
        return Response(serializer.data)
...
class FinesDetailDelete(APIView):
    """
    Retrieve, update or delete a user profile instance.
    """
    def get_object(self, pk):
        try:
            return Fines.objects.get(pk=pk)
        except Fines.DoesNotExist:
            raise Http404

    def get(self, request, pk, format=None):
        user_profile = self.get_object(pk)
        serializer = FinesSerializer(user_profile)
        return Response(serializer.data)

    def put(self, request, pk, format=None):
        user_profile = self.get_object(pk)
        serializer = FinesSerializer(user_profile, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class ExtrasListCreate(APIView):
    """
    List all user profiles or create a new user profile.
    """
    def get(self, request, format=None):
        user_profiles = ExtraTime.objects.all()
        serializer = ExtraTimeSerializer(user_profiles, many=True)
        return Response(serializer.data)
...
class ExtrasDetailDelete(APIView):
    """
    Retrieve, update or delete a user profile instance.
    """
    def get_object(self, pk):
    ...
    def get(self, request, pk, format=None):
        user_profile = self.get_object(pk)
        serializer = ExtraTimeSerializer(user_profile)
        return Response(serializer.data)

    def put(self, request, pk, format=None):
        user_profile = self.get_object(pk)
        serializer = ExtraTimeSerializer(user_profile, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def patch(self, request, pk, format=None):
        user_profile = self.get_object(pk)
        serializer = ExtraTimeSerializer(
            user_profile, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
